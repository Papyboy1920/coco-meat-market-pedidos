...
//   ADMIN_KEY     clave privada de Portal para /admin (kill switch).
// ============================================================
...
// ---------------- Auth de tienda ----------------
if (!STORE_KEY) {
  console.warn("[coco] ⚠️  STORE_KEY no configurado — /tienda abierto (modo desarrollo). Define STORE_KEY en producción.");
}
...
app.post("/api/orders", async (req, res) => {
  ...
  if (type === "pickup") {
    pickupStore = String(pickup_store || "").toLowerCase();
    if (!VALID_STORES.includes(pickupStore)) {
      return res.status(400).json({ error: "Elige la tienda para recoger: Kendall o Eureka." });
    }
  }
  ...
  for (const line of items) {
    ...
    const price = item.pending ? 0 : item.price;
    resolved.push({
      itemId: item.id,
      name: item.name,
      price,
  ...
  const itemsTotal = Math.round(resolved.reduce((s, l) => s + l.subtotal, 0) * 100) / 100;
...
db.init().then(() => {
  app.listen(PORT, () => {
    console.log(`[coco] Servidor corriendo en http://localhost:${PORT}`);
    console.log(`[coco] Clientes: http://localhost:${PORT}/`);
    console.log(`[coco] Tienda:   http://localhost:${PORT}/tienda`);
  });
}).catch((err) => {
  console.error("[coco] ❌ No se pudo inicializar la base de datos:", err.message);
  console.error("[coco] Revisa DATABASE_URL (Postgres) o los permisos del archivo coco-meat-market.db (SQLite).");
  process.exit(1);
});
