...
async function init() {
  if (process.env.DATABASE_URL) {
    ...
    kind = "pg";
    console.log("[coco] DB: Postgres");
  } else {
    const { DatabaseSync } = require("node:sqlite");
    sdb = new DatabaseSync(path.join(__dirname, "coco-meat-market.db"));
    sdb.exec(SQLITE_SCHEMA);
    kind = "sqlite";
    console.log("[coco] DB: SQLite local (coco-meat-market.db)");
  }
  ...
  if (!(await kvGet("catalog"))) {
    ...
    console.log("[coco] Catálogo semilla cargado.");
  } else {
